...
def bandwidth_avg_array(f, ban, phi, xi_knot, p):
...
def high_plot_bandwidth_depolarization_data(f, ban, phi, xi_knot, p):
    ...
    for i in range(n):
        num_pol[i] = np.abs((bandwidth_avg_polarization(f, ban, phi[i], xi_knot, p)))
        if np.abs(num_pol[i] - 0.50) < epsilon:
            if np.abs(num_pol[i] - 0.50) < half_max:
                half_max = num_pol[i]
...
def main():
    ...
    phi = half_max1(low, ban, xi_knot, p) # got a problem lol
